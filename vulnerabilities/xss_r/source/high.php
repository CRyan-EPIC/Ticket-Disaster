<?php
// XSS Reflected - High Security Level
// Regex strips script tags but other event handlers still work. Real search results.

if (isset($_GET['name']) && $_GET['name'] !== '') {
    $name = preg_replace('/<(.*)s(.*)c(.*)r(.*)i(.*)p(.*)t/i', '', $_GET['name']);

    $html .= '<div style="margin-bottom:0.5rem;">Search results for: <strong>' . $name . '</strong></div>';

    $conn = dvwaDbConnect();
    $escaped = mysqli_real_escape_string($conn, $_GET['name']);
